...
# 设置随机数种子
setup_seed(3407)
subject_num = [i for i in range(1, 25)]
# feature = []
# label = []
# for subject_id in subject_num:
#     file_dir = r'D:\EEGRecognition\Project_2103\labeled_features\de_2s\try'
...
for i,(train_index, test_index) in enumerate(loo.split(x)):
    ...
    kpca = KernelPCA(random_state=True)
    ...
    c_matrix = confusion_matrix(np.array(test_label), np.array(tes_label))
    # print(c_matrix)
    TP = c_matrix[0][0]
    FP = c_matrix[0][1]
    FN = c_matrix[1][0]
...
io.savemat(f'./predict_label_save/hci_kpca_x_y2.mat', {'x_pred': predict_label, 'x_label': target_label})
