...
from graph_data import DEAPDataset
...
# 参数调整
def parse_args():
    parser = argparse.ArgumentParser(description='RGNN Training')
    parser.add_mutually_exclusive_group()
    parser.add_argument('--root_dir',
                        type=str,
                        default='D:\EEGRecognition\Project_2103\labeled_features\de_2s',
                        help='data root path')
...
dataset = DEAPDataset(args.root_dir, args.raw_dir, args.processed_dir, args.feature_name, channel_name, position_3d, global_connection)


# 把dataset分成32组，每组120张图，对应32个被试
list=[]
for i in range(32):
    sub_set = dataset[i * 120:(i + 1) * 120]
...
def compute(model, data_loader):
    ...
    for data in data_loader:
        ...
        logits,class_output,_ = model(data,alpha)
...
# 留一法划分训练集和测试集（跨被:）
subject_id = [i for i in range(32)]

loo = LeaveOneOut()
for n,(train_index,test_index) in enumerate(loo.split(subject_id)):
    # 搭建神经网络模型
    model = SymSimGCNNet(num_nodes=32, learn_edge_weight=True, initial_edge_weight=a.edge_attr,
                         num_features=10, num_hiddens=10).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            s_class_label = data.y1.long().to(device)
            s_domain_label = torch.zeros(len(s_class_label))
            s_domain_label = s_domain_label.long().to(device)

            # 正向传播
            s_class_output, s_class_proba, s_domain_output = model(s_data, alpha)

            loss_s_label = loss_class(s_class_output, s_class_label)+0.01*torch.norm(a.edge_attr,p=1)
            loss_s_domain = loss_domain(s_domain_output, s_domain_label)

            for data in test_loader:
                t_data = data.to(device)
                t_domain_label = torch.ones(len(t_data.y1))
                t_domain_label = t_domain_label.long().to(device)
                _, _, t_domain_output = model(t_data, alpha)
