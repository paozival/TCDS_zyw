...
def parse_args():
    parser = argparse.ArgumentParser(description='BiDANN Training')
    parser.add_mutually_exclusive_group()
    parser.add_argument('--file_dir',
                        type=str,
                        default='D:\Master\EEGRecognition\Project_2103\labeled_features\seed\de',
    ...
    parser.add_argument('--save_name',
                        type=str,
                        default='hci_dann_loo_label1.mat',
...
loo = LeaveOneOut()
subject_num = [i for i in range(1, 16)]
...
for n, (train_index, test_index) in enumerate(loo.split(subject_num)):
    ...
    test_data, test_labels = test_file[args.band_name], test_file['label']
    ...
    for i in train_index:
        ...
        tr_label = train_file['label']
        tr_data.append(train_feature)
        tr_labels.append(tr_label)
    train_data = torch.FloatTensor(np.reshape(np.array(tr_data),(1890,62,10)))
    train_label = torch.LongTensor(np.reshape(np.array(tr_labels),(1890,1)))
