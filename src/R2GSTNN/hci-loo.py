...
def parse_args():
    ...
    parser.add_argument('--save_name',
                        type=str,
                        default='hci_r2gstnn_loo_try.mat',
...
for n, (train_index, test_index) in enumerate(loo.split(subject_num)):
    ...
    for i in train_index:
        ...
        tr_label = train_file[args.label_name]
        tr_data.append(train_feature)
        tr_labels.append(tr_label)

    train_data = torch.FloatTensor(np.reshape(np.array(tr_data),(1380,32,10)))
    train_label = torch.LongTensor(np.reshape(np.array(tr_labels),(1380,1)))
